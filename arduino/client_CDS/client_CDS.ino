...
void httpclient(String char_input){
  ...
  Serial.println(cmd.length());                                 // 전송 데이터 길이 확인 (디버그)
...
void setup()
{
  ...
  connectWifi();                    // Wi-fi 연결 함수 호출
  connectTCP();                     // TCP 연결 함수 호출
}

// 메인 루프
void loop()
{
  int readValue = analogRead(A0);                   // A0 핀에서 아날로그 값 읽기 (CDS)

  // 전송할 데이터 생성
  String str_output = String(readValue*10+1);       // A0 값에 계산 적용, 전송 데이터 생성

  // 서버로 데이터 전송
  httpclient(str_output);                           // 데이터 전송 함수 호출
